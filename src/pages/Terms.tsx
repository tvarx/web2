...
import { List, ShieldAlert, CornerDownLeft, CornerDownRight, Scale } from "lucide-react";
...
export function Terms() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      for (const section of t.termsPage.sections) {
  ...
  return (
    <>
      ...
      <section className={`min-h-screen pt-32 pb-24 bg-[#050505] relative overflow-hidden ${
        ...
        {/* Lights */}
        <div className="absolute top-[10%] left-[10%] w-[450px] h-[450px] rounded-full bg-[#7C3AED]/3 blur-[140px] pointer-events-none" />
        <div className="absolute bottom-[10%] right-[10%] w-[350px] h-[350px] rounded-full bg-[#A855F7]/5 blur-[120px] pointer-events-none" />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          
          {/* Header block */}
          <div className="border-b border-white/5 pb-10 mb-12 max-w-4xl">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-xs text-amber-400 mb-4">
              <Scale className="w-3.5 h-3.5" />
              <span>{t.termsPage.title}</span>
            </div>
                  ...
                  return (
                    ...
                    >
                      <span className="truncate flex items-center gap-1">
                        {isWaiver && <ShieldAlert className="w-3.5 h-3.5 text-amber-500" />}
                        {section.title}
                      </span>
              ...
              {t.termsPage.sections.map((section) => {
                ...
                return (
                  <motion.article
                    ...
                    className={`p-8 rounded-3xl border backdrop-blur-md space-y-4 scroll-mt-28 ${
                      isWaiver 
                        ? "bg-amber-950/5 border-amber-500/20" 
                        : "bg-zinc-950/30 border-white/5"
                    }`}
                  >
                    <h2 className="text-xl sm:text-2xl font-bold text-white border-b border-white/5 pb-2 flex items-center gap-2">
                      {isWaiver && <ShieldAlert className="w-6 h-6 text-amber-400" />}
                      {section.title}
                    </h2>
                    <div className={`space-y-3.5 text-sm sm:text-base leading-relaxed ${
                      isWaiver ? "text-amber-200/70 font-medium" : "text-zinc-400"
                    }`}>
